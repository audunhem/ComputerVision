...
def weights_init(module):
    if type(self.feature_extractor) == nn.Conv2d:
...
class network1(nn.Module):
    ...
    def __init__(self,
        ...
        self.num_output_features = num_filters_3*16
...
class Trainer:
    ...
    def __init__(self):
        ...
        self.optimizer = torch.optim.Adam(self.model.parameters(),self.learning_rate, (0.99,0.999), eps=1e-1)
        #self.optimizer = torch.optim.ASGD(self.model.parameters(), self.learning_rate)
        self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, 3, gamma=0.25, last_epoch=-1)
        # Load our dataset
